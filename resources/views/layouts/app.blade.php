...
<body class="bg-paper text-ink min-h-screen pb-24">
        ...
        <div class="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
            <a href="{{ auth()->check() ? route('dashboard') : route('landing') }}" class="flex items-center gap-2.5">
                <span class="inline-flex w-8 h-8 rounded-lg bg-navy items-center justify-center">
                    <svg class="w-4.5 h-4.5 text-white" width="18" height="18" viewBox="0 0 24 24" fill="none"
                ...
                <div class="leading-tight">
                    <div class="font-bold text-[15px] tracking-tight">
                        {{ auth()->check() ? auth()->user()->company->name : 'BatchDesk' }}</div>
            ...
            @auth
                @if (auth()->user()->is_admin)
                    <a href="{{ route('admin.dashboard') }}"
                        class="text-xs font-semibold text-brand border border-line rounded-lg px-3 py-1.5 hover:bg-paper">Admin</a>
                @endif
                <a href="{{ route('settings.index') }}"
                    class="text-xs font-medium text-muted border border-line rounded-lg px-3 py-1.5 hover:bg-paper">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button
                        class="text-xs font-medium text-muted border border-line rounded-lg px-3 py-1.5 hover:bg-paper">Sign
                        out</button>
                </form>
            @endauth
        </div>
    ...
    @if (session('success'))
        <div class="no-print max-w-3xl mx-auto px-4 mt-3">
            <div
                class="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg px-4 py-3 text-sm font-medium flex items-center gap-2">
    ...
    @auth
        @if (!auth()->user()->is_admin && auth()->user()->company && auth()->user()->company->isTrialActive())
            ...
            @if ($days <= 10)
                <div class="no-print max-w-3xl mx-auto px-4 mt-3">
                    <div
                        class="bg-amber-50 border border-amber-300 rounded-lg px-4 py-2.5 text-sm flex items-center justify-between gap-3">
                        <span class="text-amber font-medium">
                            ⏰ Your free trial ends in <strong>{{ $days }}
                                {{ $days === 1 ? 'day' : 'days' }}</strong>.
                        </span>
                        <a href="https://wa.me/?text=Hi%2C+I+want+to+subscribe+to+BatchDesk" target="_blank"
                            class="text-xs font-bold text-amber underline whitespace-nowrap">Contact us to subscribe →</a>
                    </div>
                </div>
            @endif
        @endif
        ...
        <nav class="no-print fixed bottom-0 inset-x-0 bg-white border-t border-line z-40">
            <div class="max-w-3xl mx-auto grid grid-cols-6 text-center text-[11px] font-medium">
                ...
                <a href="{{ route('batches.index') }}"
                    class="py-2.5 {{ request()->routeIs('batches.*') ? $active : $idle }}">
                    <svg class="w-5 h-5 mx-auto mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M9 3h6m-5 0v6.34L5.42 16.1A2.5 2.5 0 0 0 7.6 20h8.8a2.5 2.5 0 0 0 2.18-3.9L14 9.34V3" />
                    </svg>
                    Batches
                </a>
                <a href="{{ route('quotations.index') }}"
                    class="py-2.5 {{ request()->routeIs('quotations.*') ? $active : $idle }}">
                    <svg class="w-5 h-5 mx-auto mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <a href="{{ route('sales.create') }}" class="py-1 -mt-4">
                    <div
                        class="mx-auto w-12 h-12 rounded-xl bg-navy text-white flex items-center justify-center shadow-lg shadow-slate-300">
                ...
                <a href="{{ route('customers.index') }}"
                    class="py-2.5 {{ request()->routeIs('customers.*') || request()->routeIs('sales.index') ? $active : $idle }}">
                    <svg class="w-5 h-5 mx-auto mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
