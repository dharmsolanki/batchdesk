...
@section('content')
    ...
    @php
        ...
        $coaLinks = $sale->items
            ->filter(fn($i) => $i->batch && $i->batch->status === 'released')
            ->map(fn($i) => $i->batch->batch_no . ': ' . route('coa.verify', $i->batch->coa_token))
            ->implode("\n");
        $waText = rawurlencode(
            '*' .
                $sale->company->name .
                ...
                $sale->items
                    ->map(
                        fn($i) => $i->description .
                            "\n" .
                            rtrim(rtrim(number_format((float) $i->qty, 3), '0'), '.') .
                            ' ' .
                            $i->unit .
                            ' = ₹' .
                            number_format((float) $i->line_total),
                    )
                    ->implode("\n") .
                "\n" .
                "------------------\n" .
                '*Total: ₹' .
                number_format((float) $sale->total) .
                "*\n" .
                $sale->payments
                    ->map(
                        fn($p) => 'Paid (' .
    ...
    @if ($sale->pending > 0)
        <div class="no-print card border-amber-300 bg-amber-50 p-4 mb-4">
            ...
            <form method="POST" action="{{ route('sales.payment', $sale) }}" class="flex gap-2">
                @csrf
                <input name="amount" type="number" step="0.01" min="1" max="{{ $sale->pending }}" required
                    placeholder="Amount received" class="field flex-1 min-w-0">
    ...
    <div class="card p-7">
            ...
            <div>
                ...
                <div class="text-xs text-muted mt-0.5">
                    ...
                    @if ($sale->company->gst_number)
                        <br>GSTIN: {{ $sale->company->gst_number }}
                    @endif
                </div>
            </div>
        ...
        <div class="py-3 border-b border-line/60 text-sm">
            ...
            @if ($sale->customer->gst_number)
                <span class="text-xs text-muted"> · GSTIN: {{ $sale->customer->gst_number }}</span>
            @endif
            @if ($sale->customer->address)
                <div class="text-xs text-muted">{{ $sale->customer->address }}</div>
            @endif
        </div>
        ...
        <div class="mt-4 ml-auto max-w-xs text-sm space-y-1">
            <div class="flex justify-between text-muted">
                <span>Subtotal</span><span>₹{{ number_format((float) $sale->subtotal, 2) }}</span>
            </div>
            <div class="flex justify-between text-muted">
                <span>CGST</span><span>₹{{ number_format((float) $sale->gst_amount / 2, 2) }}</span>
            </div>
            <div class="flex justify-between text-muted">
                <span>SGST</span><span>₹{{ number_format((float) $sale->gst_amount / 2, 2) }}</span>
            </div>
            <div class="flex justify-between font-bold text-lg border-t border-line pt-2">
                <span>Total</span><span>₹{{ number_format((float) $sale->total, 2) }}</span>
            </div>
        </div>
        ...
        <div class="mt-4 border-t border-line/60 pt-3">
            ...
            @forelse ($sale->payments->sortBy('created_at') as $p)
                <div class="flex justify-between text-sm py-1">
                    <span class="text-slate-600">{{ $p->created_at->format('d M Y, h:i a') }} · <span
                            class="font-semibold text-brand">{{ $modeLabels[$p->mode] ?? strtoupper($p->mode) }}</span></span>
            ...
            @if ($sale->pending > 0)
                <div class="flex justify-between text-sm font-bold text-amber"><span>Balance
                        due</span><span>₹{{ number_format($sale->pending, 2) }}</span></div>
            @endif
        </div>
        ...
        @if ($coaBatches->count())
            <div class="no-print mt-4 border-t border-line/60 pt-3">
                ...
                @foreach ($coaBatches as $item)
                    <a href="{{ route('batches.coa', $item->batch) }}"
                        class="block text-brand font-semibold text-sm py-1">COA — <span
                            class="font-mono">{{ $item->batch->batch_no }}</span> →</a>
                @endforeach
            </div>
        @endif
        ...
        <div class="mt-6 text-center text-[10px] text-muted">This is a computer-generated invoice · Generated by BatchDesk
        </div>
    </div>
@endsection
